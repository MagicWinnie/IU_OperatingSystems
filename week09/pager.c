...
#include <sys/mman.h>
#include <sys/stat.h>
#define PAGETABLE_FILENAME "/tmp/ex1/pagetable"
#define PID_FILENAME "/tmp/ex1/pager_pid"
...
size_t P, F;
int algorithm; // 0 - random, 1 - NFU, 2 - aging
char **disk, **RAM;
...
int nfu()
{
    ...
    for (int i = 0; i < P; i++)
    {
        if (pagetable[i].referenced)
            nfu_counter[i]++;
        if (nfu_counter[i] < min_value)
        {
            min_index = i;
            min_value = nfu_counter[i];
        }
    }
    return min_index;
}
...
int aging()
{
    ...
    for (int i = 0; i < P; i++)
    {
        aging_counter[i] = (aging_counter[i] >> 1) | (pagetable[i].referenced << 7);
        if (aging_counter[i] < min_value)
        {
            min_index = i;
            min_value = aging_counter[i];
        }
    }
    return min_index;
}
...
void handle_sigusr1(int code)
{
    ...
    if (mmu_pid == 0)
    {
        ...
        if (algorithm == 0)
            printf("Have used random page replacement algorithm\n");
        else if (algorithm == 1)
        {
            printf("Have used nfu page replacement algorithm\n");
            free(nfu_counter);
        }
        else if (algorithm == 2)
        {
            printf("Have used aging page replacement algorithm\n");
            free(aging_counter);
        }
...
int main(int argc, char *argv[])
{
    if (argc != 4)
    {
        fprintf(stderr, "[ERROR] Usage: ./pager.out <number of pages> <number of frames> <page replacement algorithm>\n");
        exit(EXIT_FAILURE);
    }
    ...
    {
        algorithm = 1;
        nfu_counter = (int *)calloc(P, sizeof(int));
    }
    else if (strcmp(argv[3], "aging") == 0)
    {
        algorithm = 2;
        aging_counter = (unsigned char *)calloc(P, sizeof(unsigned char));
    }
    else
    {
        fprintf(stderr, "[ERROR] Unknown page replacement algorithm type\n");
        exit(EXIT_FAILURE);
    }
